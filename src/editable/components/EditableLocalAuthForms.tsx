...
const inputClass = 'h-[3.25rem] rounded-lg border border-slate-900/10 bg-[#f8fbf7] px-4 text-base font-bold text-[var(--slot4-page-text)] outline-none transition placeholder:text-current/35 focus:border-[var(--slot4-accent)]'
const buttonClass = 'inline-flex h-[3.25rem] items-center justify-center rounded-lg bg-[var(--slot4-accent)] px-6 text-xs font-black uppercase tracking-[0.18em] text-white transition hover:bg-[var(--slot4-coral)] disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-lg border px-4 py-3 text-sm font-bold ${status === 'success' ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-red-200 bg-red-50 text-red-700'}`}>{message}</p> : null}
